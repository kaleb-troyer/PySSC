...
class C_sco2_sim:
    ...
    def solve_sco2_case(self, display=True):
        ...
        if self.m_is_print_key_outputs and display:
            print(f'{"solve time":.<35}{self.solve_time:.>10.4f}')
            self.print_sco2_case()

    def print_sco2_case(self): 

        display = [
            ("The cycle efficiency is", "eta_thermal_calc"),
            # ("The net power output is", "W_dot_net_less_cooling"),
            # ("The net compressor power is", "c_tot_W_dot"),
            # ("The net turbine power is", "t_W_dot"),

            # ("The cycle cost per kW ($/kWe) is", "cycle_spec_cost"),
            # ("The total cost per kW ($/kWe) is", "total_spec_cost"),

            # ("The cycle mass flow rate is", "m_dot_co2_full"),
            # ("The recompression fraction is", "recomp_frac"),
            ("The receiver efficiency is", "receiver_efficiency"), 
            # ("The PHX temp diff", "deltaT_HTF_PHX"),
            # ("The PHX effectiveness", "eff_PHX"),
            # ("The PHX NTU", "NTU_PHX"),
            # ("The PHX UA", "UA_PHX"),
            # ("The PHX power is", "q_dot_PHX"), 
            ("The HTF mass flow rate", "m_dot_htf_des"),
            ("The HTF dT across PHX is", "deltaT_HTF_PHX"), 
            # ("The HTF volumetric flow rate", "V_dot_htf_des"),
            # ("sCO2 PHX inlet temp", "T_co2_PHX_in"),
            # ("sCO2 PHX leave temp", "T_turb_in"),

            ("The LTR UA fraction is", "recup_LTR_UA_frac"),
            ("The LTR UA is", "LTR_UA_calculated"),
            ("The HTR UA is", "HTR_UA_calculated"),

            # ("Solar tower cost", "solar_tower_cost"),
            # ("Solar field cost", "solar_field_cost"),
            # ("FPR cost", "falling_particle_receiver"),
            # ("Particle cost", "particles_cost"),
            # ("Particle loss cost", "particle_losses_cost"),
            # ("TES cost", "particle_storage_cost"),
            # ("Lift cost", "particle_lifts_cost"),
            # ("Land cost", "land_cost"),
            
            # ("Turbine cost", "turbine_capital_cost"),
            # ("Compressor cost, main", "compressor_capital_cost"),
            # ("Compressor cost, re", "recompressor_capital_cost"),
            # ("Piping costs, etc.", "piping_inventory_etc_cost"),
            # ("Air cooler cost", "air_cooler_capital_cost"),
            # ("BOP cost", "balance_of_plant_cost"),
            # ("PHX cost", "PHX_capital_cost"),
            # ("LTR cost", "LTR_capital_cost"),
            # ("HTR cost", "HTR_capital_cost"),

            # ("The annual O&M cost is", "annual_maintenance_cost"),
        ...
        buffer = 35
        print(f'{"heat transfer fluid":.<{buffer}}{self.m_des_par_sim["htf"]:.>10.4f}')
        for param in display:
            print(f'{param[0]:.<{buffer}}{self.m_solve_dict[param[1]]:.>10.4f}')  
        print()
    ...
    def save_m_solve_dict(self, s_file_name):
        ...
        j_output = open('SSC CSP API/results/' + s_file_name + ".json", 'w')
            
        json.dump(self.m_solve_dict, j_output)
        j_output.close()
        
        if(self.m_also_save_csv):
            with open('SSC CSP API/results/' + s_file_name + '.csv','w', newline='') as f:
                w = csv.writer(f)
...
def sim_data__eta_vs_UA_vs_config(UA_par_in, is_save_data = False, file_name = ""):
    ...
    if(is_save_data):
        ...
        for cycle_i in [RC_des_solved, PC_des_solved]:
            ...
            if(file_name == ""):
                j_output = open('results/' + cycle_name + "_UA_parametric.txt",'w')
            else:
                j_output = open('results/' + cycle_name + "_" + file_name + "_UA_parametric.txt", 'w')
...
def sim_and_save_data__deltaT_UA_config(deltaT_list, UA_list, is_save_file = True, file_name = "", cycle_config = "", mod_base_dict = {}):
    ...
    for k in range(len(cycle_config_par)):
        ...
        if(is_save_file):
            ...
            output = open('results/' + cycle_name + file_name + ".txt",'w')
                
            json.dump(des_results[k], output)
            output.close()
            
            "Print to csv so we easily review results"
            with open('results/' + cycle_name + file_name + ".csv",'w', newline='') as f:
                w = csv.writer(f)
...
def compare_two_json_files(json_in_1, desc_1, json_in_2, desc_2, comp_txt_out):
    ...
    outfile = open('results/' + comp_txt_out, "w")
...
class C_process_and_plot_udpc:
    ...
    def process_and_plot(self):
        ...
        with open('results/' + self.settings.desc + "_udpc_outputs" + '.csv', 'w', newline='') as f:
            w = csv.writer(f)
